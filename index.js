...
// const path = require('path');
// const { CourseStudent } = require('./models/model');
// const { StudentList } = require('./models/model');
...
// Training Form Schema and Routes
const formSchema = new mongoose.Schema({
  name: String,
  phone: String,
  email: String,
...
app.get('/api/colleges', async (req, res) => {
  try {
    const colleges = await mongoose.model('College').find({}, { _id: 1, name: 1 });
    console.log("college", colleges);
    res.json(colleges);
  } catch (error) {
    res.status(500).json({ message: error.message });
...
app.get('/api/interns/count', async (req, res) => {
  try {
    const totalCount = await Intern.countDocuments();
    console.log("Total Interns Count:", totalCount);
    res.status(200).json({ total: totalCount });
  } catch (error) {
    console.error("Error fetching count:", error);
...
app.post("/api/interns", async (req, res) => {
  try {
    const lastIntern = await Intern.findOne().sort({ S_NO: -1 });
    const newSNo = lastIntern ? lastIntern.S_NO + 1 : 1;
    console.log("intern", req.body)
    ...
    const newIntern = new Intern({
      ...
      Certificate_Type_id: req.body.Certificate_Type_id || req.body.certificate_type_id,
      yop: req.body.yop,
      Role: req.body.Role,
      From: req.body.From,
      To: req.body.To,
...
app.post('/api/course', async (req, res) => {
  console.log("Course new data:", req.body);
  try {
    const newCourse = new Course(req.body);
    await newCourse.save();
    res.status(201).json(newCourse);
...
app.post('/api/subject', async (req, res) => {
  try {
    console.log("sree", req.body)
    const { SubjectId, SubjectName, coursesids, SubjectCaption, SubjectDesc } = req.body;
    console.log("Subject new data:", req.body);
...
// Registration Schema and Routes
const PaymentPlanSchema = new mongoose.Schema({
  dueDate: String,
  amount: Number,
  status: { type: String, default: "Pending" },
